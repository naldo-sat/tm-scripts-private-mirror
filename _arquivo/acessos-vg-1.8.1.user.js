...
(function() {
    ...
    // Item 1 — flag por conta (extração única por sessão)
    const _contasExtraidas = new Set();

    // Item 1 — timestamp de sessão: restaurado do sessionStorage (sobrevive à
    // navegação login → portal) e marcado na inicialização do portal
    ...
    // ─── MAPEAMENTO CONTA → ABA DE DADOS ─────────────────────────────────────────
    const abasDadosPorConta = {
        "0455828133": "NALDO SAT - Dados",
        "0459325639": "NALDO SAT - Dados",
        "0453979554": "NALDO SAT - Dados",
        "0444346918": "NALDO SAT - Dados",
        "0450619128": "NALDO SAT - Dados",
        "0452109744": "STUDIO ML - Dados",
        "0454860388": "STUDIO ML - Dados",
        "0444225746": "STUDIO ML - Dados",
        "0457460616": "STUDIO ML - Dados",
        "0462105797": "STUDIO ML - Dados",
        "0466121938": "STUDIO ML - Dados",
        "0463297834": "F DE ASSIS - Dados",
        "0451176465": "F DE ASSIS - Dados",
        "0443889484": "F DE ASSIS - Dados",
        "0461401781": "F DE ASSIS - Dados",
        "0469102728": "CONNECTA - Dados",
        "0469103350": "CONNECTA - Dados",
        "0468571160": "CN ENGENHARIA - Dados",
        "0469296149": "CN ENGENHARIA - Dados",
        "0469301552": "CN ENGENHARIA - Dados",
        "0469288595": "CN ENGENHARIA - Dados"
    };

    // ─── CAPTURA DE SESSÃO + DETECÇÃO DE LOGIN ──────────────────────────────────
    let _sessaoCapturada = null;
    (function capturarSessaoELogin() {
        ...
        unsafeWindow.XMLHttpRequest.prototype.open = function(method, url) {
            ...
            this._csUrl    = url || '';
            // Captura parâmetros de sessão do loadView para chamada GM_xmlhttpRequest
            if (url && url.includes('datapackconsumption') && url.includes('action=loadView')) {
                try {
                    ...
                    _sessaoCapturada = {
                        ...
                        acessLogin: params.get('acessLogin') || '',
                    };
                } catch (_) {}
            }
    ...
    // ─── CLIQUE EM "CONSUMO DE DADOS" → CHAMADA PRÓPRIA ──────────────────────────
    // Não compete com o hook do MoveLines — GM_xmlhttpRequest roda fora da página.
    // Dispara apenas no clique explícito do usuário.
    document.addEventListener('click', function(e) {
        const link = e.target.closest('a.anchor-context');
        if (!link) return;
        if (!/consumo\s+de\s+dados/i.test(link.textContent)) return;

        // Aguarda o Angular completar a requisição antes de fazer a nossa
        setTimeout(function() {
            const conta = obterContaAtiva();
            if (!conta) {
                console.warn('⚠️ Conta ativa não identificada.');
                return;
            }

            // Item 1: marca imediatamente para bloquear cliques múltiplos
            // (flag definida antes do delay — sem corrida entre cliques rápidos)
            if (_contasExtraidas.has(conta)) {
                console.log('ℹ️ Dados de ' + conta + ' já extraídos nesta sessão. Ignorando.');
                return;
            }
            _contasExtraidas.add(conta); // bloqueia novos cliques imediatamente

            if (!_sessaoCapturada || !_sessaoCapturada.sessionId) {
                console.warn('⚠️ Sessão ainda não capturada. Tente novamente.');
                return;
            }

            const url = 'https://vivogestao.vivoempresas.com.br/Portal/api/datapackconsumption' +
                '?action=loadView' +
                '&technology=4G' +
                '&startRow=1' +
                '&fetchSize=500' +
                '&sessionId='  + encodeURIComponent(_sessaoCapturada.sessionId)  +
                '&remoteHost=' + encodeURIComponent(_sessaoCapturada.remoteHost) +
                '&remoteIp='   + encodeURIComponent(_sessaoCapturada.remoteIp)   +
                '&acessLogin=' + encodeURIComponent(_sessaoCapturada.acessLogin);

            console.log('📡 Buscando dados de consumo para conta:', conta);
            ...
            GM_xmlhttpRequest({
                ...
                onload: function(response) {
                    try {
                        ...
                        processarDadosConsumo(conta, resposta);
                    } catch (e) {
                        console.error('❌ Erro ao processar resposta de consumo:', e);
                    }
                },
                onerror: function(e) {
                    // Remove flag para permitir nova tentativa em caso de falha de rede
                    _contasExtraidas.delete(conta);
                    console.error('❌ Falha na requisição de consumo:', e);
                }
            });
        }, 2000); // aguarda 2s para o Angular completar o loadView
    }, true);

    // ─── PROCESSAMENTO DOS DADOS DE CONSUMO ──────────────────────────────────────
    function processarDadosConsumo(conta, resposta) {
        // conta vem de obterContaAtiva() — número real de 10 dígitos do dropdown
        const aba = abasDadosPorConta[conta];
        if (!aba) {
            console.warn('⚠️ Conta sem aba de dados mapeada:', conta);
            return;
        }
        const contaNormalizada = conta;

        const grupos = resposta.groupList || [];
        const linhas = [];

        grupos.forEach(grupo => {
            const nomeGrupo = (grupo.name || '').trim();
            const linhasGrupo = grupo.lines || [];

            linhasGrupo.forEach(linha => {
                // Ignora linhas já movidas (histórico interno do sistema Vivo)
                if (String(linha.blockConsumptionStatus) === '1') return;

                linhas.push({
                    nome:       (linha.userName     || '').trim(),
                    linha:      (linha.lineNumber    || '').trim(),
                    cota:       (linha.quota?.value  || '0') + ' GB',
                    cotaUsada:  (linha.quotaConsumption?.value || '0') + ' GB',
                    percentual: (linha.percentageConsumedQuota || '0%').trim(),
                    grupo:      nomeGrupo,
                    conta:      conta
                });
            });
        ...
        if (linhas.length === 0) {
            console.warn('⚠️ Nenhuma linha válida encontrada para conta:', conta);
            return;
        }

        console.log('📊 Enviando ' + linhas.length + ' linhas para aba "' + aba + '"...');
        enviarDadosParaPlanilha(contaNormalizada, aba, linhas);
    }

    // ─── ENVIO DOS DADOS AO APPS SCRIPT ──────────────────────────────────────────
    function enviarDadosParaPlanilha(conta, aba, linhas) {
        GM_xmlhttpRequest({
            method:  'POST',
            url:     WEB_APP_URL,
            headers: { 'Content-Type': 'application/json' },
            data:    JSON.stringify({ tipo: 'dados', conta, aba, linhas }),
            onload: function(response) {
                try {
                    ...
                    if (resultado.success) {
                        ...
                        console.error('❌ Erro ao gravar dados: ' + resultado.error);
                    }
                } catch (e) {
                    console.error('❌ Erro ao parsear resposta de dados:', e);
                }
            },
            onerror: function(e) {
                console.error('❌ Falha na requisição de dados ao Web App:', e);
            }
        });
    }
    ...
    function preencherLogin(conta) {
        ...
        // Reseta flag de extração da conta selecionada
        _contasExtraidas.delete(conta.nome);
    ...
    document.addEventListener('click', function(e) {
        ...
        const conta = obterContaAtiva();
        // Item 1: reseta flag da conta ao fazer logout
        if (conta) _contasExtraidas.delete(conta);
